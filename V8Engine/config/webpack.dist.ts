...
function GenerateConfig(entry: Entry, filename: string, mode: "development" | "production" | "none", target: string, index: boolean): webpack.Configuration {
	...
	let config: webpack.Configuration = {
		...
		}, mode: mode, devtool: "source-map", module: {rules: rules}
	};
	if (target === "node") {
		config.target = "node";
...
module.exports = [GenerateConfig(entry, '[name].min.js', 'production', target, false), GenerateConfig(entry, '[name].js', 'development', target, false), GenerateConfig(entry, 'index.js', 'production', target, true)];